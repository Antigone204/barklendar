...
final Provider<AiClient?> aiServiceProvider = Provider<AiClient?>((ProviderRef<AiClient?> ref) {
  // 监听激活配置的变化
  ...
  try {
    ...
    final String? type = activeConfig[AiConfigKeys.type];
    if (type == 'generic') {
      return GenericOpenAiCompatibleClient(activeConfig);
    }

    final String identifier = activeConfig[AiConfigKeys.type] ?? 'openai';
    switch (identifier) {
      case 'openai':
