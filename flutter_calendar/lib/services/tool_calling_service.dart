...
''';
      ...
      // 步骤4: 首次LLM调用 - 思考阶段
      final AIService aiService = _ref.read(aiServiceProvider);
      final AIResponse thinkingResponse = await aiService.generateResponseWithTools(
        messages: messages.map((ChatMessage m) => m.toOpenAIMessage()).toList(),
        tools: toolSchemas,
      );
  ...
  Future<String> _handleFunctionCall(
    ...
    try {
      // 步骤1: 验证函数调用
      final Map<String, dynamic>? validation = _toolRegistry.validateFunctionParameters(
        functionCall.name,
      ...
      try {
        final ToolCallRequest request = ToolCallRequest(
      ...
      // 步骤3: 构建总结阶段的系统提示
      final String summarizingSystemPrompt = '''
...
''';
      ...
      // 步骤4: 二次LLM调用 - 总结阶段
      final List<ChatMessage> summarizingMessages = <ChatMessage>[
        ChatMessage(
            role: 'system',
      ...
      final AIService aiService = _ref.read(aiServiceProvider);
      final String summarizingResponse = await aiService.generateResponse(
        messages: summarizingMessages.map((ChatMessage m) => m.toOpenAIMessage()).toList(),
      );
  ...
  Stream<AiTurnState> executeTurnStream(String userMessage,
    ...
    final List<ChatMessage> conversationHistory = <ChatMessage>[
      ...history,
      ChatMessage(
          role: 'user', content: userMessage, timestamp: DateTime.now(),),
    ];

    const int maxTurns = 5; // 设定一个最大循环次数，防止无限循环
    ...
    try {
      ...
      for (int turn = 0; turn < maxTurns; turn++) {
        ...
        final AIService aiService = _ref.read(aiServiceProvider);
        final AIResponse response = await aiService.generateResponseWithTools(
          messages: messagesForApi.map((ChatMessage m) => m.toOpenAIMessage()).toList(),
          tools: toolSchemas,
        );
        ...
        if (response.hasFunctionCall) {
          ...
          // 执行工具
          final ToolCallResponse toolResponse =
  ...
  Stream<AiTurnState> _streamDirectResponse(String content) async* {
    ...
    for (int i = 0; i < words.length; i++) {
      final String chunk = words.sublist(0, i + 1).join(' ');
...
''';
    ...
    // 模拟流式总结输出
    final AIService aiService = _ref.read(aiServiceProvider);
    final String summaryContent = await aiService.generateResponse(
      messages: summarizingMessages.map((ChatMessage m) => m.toOpenAIMessage()).toList(),
    );
...
''';
  ...
  Map<String, dynamic> getServiceStatistics() {
    final Map<String, dynamic> toolStats = _toolRegistry.getToolStatistics();
    return <String, dynamic>{
      'toolCallingService': <String, dynamic>{
